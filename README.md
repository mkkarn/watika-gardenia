🌸 Watika Gardenia — Restaurant Web Application

A full-featured, production-ready restaurant ordering platform built with React 18. Order food online, track deliveries live, and enjoy a seamless dining experience from your browser.


🍽️ What Is This Project?
Watika Gardenia is a modern restaurant web application designed for online food ordering, delivery management, and customer engagement. It simulates a real-world restaurant digital storefront — complete with a product catalogue, shopping cart, multi-step checkout, live order tracking, and a CI/CD DevSecOps pipeline ready for deployment.
Built as a full frontend SPA (Single Page Application) using React 18, it is structured to be easily extended, deployed via Docker, and integrated into any cloud pipeline.

✨ Key Highlights

🛒 End-to-end ordering flow — browse menu → add to cart → checkout → order confirmation
🔴 Live order tracker — animated real-time status updates (Confirmed → Preparing → Out for Delivery → Delivered)
🎟️ Coupon engine — 4 working discount codes with percentage and flat-rate support
📦 Persistent cart — cart survives page refresh via localStorage
🎯 Smart recommendations — context-aware dish suggestions across all pages
📢 Ad banner rotator — auto-cycling promotional banners with coupon codes
🔒 Secure checkout — form validation, card/UPI/COD payment options, SSL badge
🌙 Dark luxury theme — gold accent design system built for elegance
📱 Responsive design — works across desktop, tablet, and mobile
🐳 Docker-ready — multi-stage build with Nginx, non-root user, security headers
⚙️ DevSecOps pipeline — GitHub Actions CI/CD with SAST, secret scanning, and Trivy image scanning
...
├── src/
│   ├── components/
│   │   ├── Navbar.js               # Sticky navigation with cart counter
│   │   ├── Footer.js               # Links, contact info, newsletter signup
│   │   ├── MenuItemCard.js         # Product card with add/remove controls
│   │   └── Recommendations.js      # Dynamic dish suggestion component
│   │
│   ├── context/
│   │   ├── CartContext.js          # Global cart state, coupon logic, pricing
│   │   └── OrderContext.js         # Order placement and history management
│   │
│   ├── data/
│   │   └── menuData.js             # 20+ menu items across 7 categories
│   │
│   ├── pages/
│   │   ├── HomePage.js             # Hero, ad banners, featured dishes
│   │   ├── MenuPage.js             # Full catalogue with search and filters
│   │   ├── CartPage.js             # Cart review, coupons, order summary
│   │   ├── CheckoutPage.js         # 3-step checkout (Delivery→Payment→Review)
│   │   ├── OrderConfirmationPage.js# Animated live order tracker
│   │   └── OrderHistoryPage.js     # Past orders with reorder option
│   │
...
├── .github/
│   └── workflows/
│       └── ci-cd.yml               # Full DevSecOps pipeline (5 jobs)
│
├── Dockerfile                      # Multi-stage production build
├── docker-compose.yml              # Dev + prod container profiles
...
🗂️ Menu Categories
CategoryItems🥗 StartersMezze Platter, Calamari, Paneer Tikka, Wings, Soup🍛 Main CourseButter Chicken, Grilled Sea Bass, Lamb Rogan Josh, Thali🫓 BreadsGarlic Naan, Stuffed Paratha🍚 Rice & BiryaniHyderabadi Dum Biryani, Veg Pulao🍮 DessertsGulab Jamun, Chocolate Fondant, Phirni🥤 BeveragesMango Lassi, Gardenia Mocktail, Filter Coffee⭐ Chef SpecialsWatika Signature Platter, Truffle Risotto

🎟️ Coupon Codes
CodeOfferWELCOME2020% off — new customer discountWATIKA1010% off — loyalty offerFLAT100₹100 flat off on orders above ₹500GARDENIA15% off — seasonal special

💳 Payment Methods Supported

Credit / Debit Card — with real-time validation (number, expiry, CVV)
UPI — GPay, PhonePe, Paytm (UPI ID entry)
Cash on Delivery — with delivery note

🏗️ Tech Stack
LayerTechnologyFrontendReact 18, React Router v6State ManagementReact Context API + useReducerStylingCSS Variables, custom design systemIconsLucide ReactNotificationsReact Hot ToastContainerisationDocker (multi-stage), Nginx 1.25CI/CDGitHub ActionsSecurity ScanningSemgrep, TruffleHog, Trivy, npm auditRegistryGitHub Container Registry (GHCR)
...
>---
>
># Watika Gardenia = Microservices in Action
>
>This restaurant app is made up of multiple services like:
>
>### 🧾 Menu ServiceServes all dish data 
>
>- categories, prices, images, filters
>
>### 🛒 Cart Service
>
>- Manages cart state, quantities, coupon validation, pricing
>
>### 💳 Payment Service
>
>- Handles Card, UPI, COD flows and payment validation
>
>### 📦 Recommendation Service 
>
>- Suggests dishes based on ratings, category, cart context
>
>### Delivery/Tracking Service
>
>- Tracks order status in real time (4-stage pipeline)
>
>### Promotion/Ad Service 
>
>- Rotates banners, manages coupon codes and discounts
>
>### User/Auth Service 
>
>- Manages customer profiles, addresses, preferences
>
>### Search Service
>
>- Powers menu search by name, tag, ingredient, category
>
> 📦 If You Were to Split Into Real Microservices
Each service would become its own independent deployable unit:
>
> watika-gardenia/
├── services/
│   ├── menu-service/          # Node.js + MongoDB — dish catalogue
│   ├── cart-service/          # Redis — ephemeral cart state
│   ├── order-service/         # Node.js + PostgreSQL — order records
│   ├── payment-service/       # Node.js — Razorpay / Stripe gateway
│   ├── recommendation-service/# Python — ML-based suggestions
│   ├── notification-service/  # Node.js — EmailJS / SendGrid
│   ├── tracking-service/      # Node.js + WebSockets — live status
│   ├── promotion-service/     # Node.js — coupon engine
│   ├── search-service/        # Elasticsearch — dish search & filter
│   └── user-service/          # Node.js + JWT — auth & profiles
│
├── api-gateway/               # Single entry point for all services
└── frontend/                  # React app — consumes all APIs
>
