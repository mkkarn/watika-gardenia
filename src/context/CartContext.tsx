...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  switch (action.type) {
    ...
    case 'APPLY_COUPON': {
      const coupon = coupons.find(c => c.code === action.payload.toUpperCase());
      return { ...state, coupon: coupon || null };
    }
    case 'REMOVE_COUPON':
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('wg_cart');
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('wg_cart', JSON.stringify(state));
  }, [state]);
  ...
  const getDiscount = (): number => {
    ...
    if (state.coupon.minOrder && total < state.coupon.minOrder) {
      return 0;
    }
  ...
  const getFinalTotal = (): number => {
    ...
    const taxes = (total - discount) * 0.05; // 5% tax
